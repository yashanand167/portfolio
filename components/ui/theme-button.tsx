  ...
  return (
    <>
      ...
      <motion.div
        ...
        dragListener={false}
        dragConstraints={canDrag ? constraintsRef : false}
        dragElastic={0.08}
        dragMomentum={false}
        whileDrag={canDrag ? { scale: 1.02 } : undefined}
        transition={spring}
        className={`pointer-events-auto fixed bottom-6 left-1/2 z-50 touch-manipulation pb-[env(safe-area-inset-bottom,0px)] ${
          canDrag ? "" : "-translate-x-1/2"
        }`}
        style={canDrag ? { x: "-50%" } : undefined}
      >
        <motion.div
          transition={spring}
